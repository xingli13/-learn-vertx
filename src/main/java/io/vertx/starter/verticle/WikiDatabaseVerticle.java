...
public class WikiDatabaseVerticle extends AbstractVerticle {
  ...
  public static final String CONFIG_WIKIDB_SQL_QUERIES_RES_FILE = "wikidb.sqlqueries.resource.file";
  ...
  private void loadSqlQueries() throws IOException {
    String queriesFile = config().getString(CONFIG_WIKIDB_SQL_QUERIES_RES_FILE);
  ...
  public void start(Future<Void> startFuture) throws Exception {
    ...
    dbClient.getConnection(ar -> {
      if (ar.failed()) {
        ...
        connection.execute(sqlQueries.get(SqlQuery.CREATE_PAGES_TABLE), creat -> {
          connection.close();
          if (creat.failed()) {
            LOGGER.error("Database preparation error", creat.cause());
            startFuture.fail(creat.cause());
          } else {
